...
7. Run `generic_review_report_consolidator` after specialist reports finish when a combined report is needed.

## Default Review Matrix

For a broad review, use these reviewer profiles:

1. `generic_code_reviewer`
2. `generic_ethical_hacking_specialist`
3. `generic_silent_failure_hunter`
4. `generic_type_design_analyzer`
5. `generic_test_automation_specialist`
6. `generic_performance_optimization_expert`
7. `generic_architecture_review_specialist`
8. `generic_plan_alignment`
9. `generic_review_report_consolidator` after reviewer reports are available
...
| --- | --- |
| API routes, auth, permissions, secrets, local processes, file/network access | `generic_ethical_hacking_specialist`, `generic_silent_failure_hunter`, `generic_type_design_analyzer` |
| Persistence, migrations, queues, idempotency, rollback, restart recovery | `generic_silent_failure_hunter`, `generic_system_resilience_testing_expert`, `generic_type_design_analyzer` |
| Async work, subprocesses, streams, callbacks, cancellation, retries, cleanup | `generic_error_analysis_and_resolution_expert`, `generic_silent_failure_hunter`, `generic_system_resilience_testing_expert` |
| New DTOs, schemas, validators, generated clients, config/state machines | `generic_type_design_analyzer`, `generic_architecture_review_specialist`, `generic_test_automation_specialist` |
| Tests, fixtures, mocks, CI scripts, validation wrappers | `generic_test_automation_framework_expert`, `generic_test_automation_specialist` |
| Performance-sensitive loops, batch jobs, DB calls, process/network fan-out | `generic_performance_optimization_expert` |
| Large design shifts, module boundaries, public contracts, plan-vs-code concerns | `generic_plan_alignment`, `generic_architecture_review_specialist` |
| AI-generated or agent-written code | `generic_code_reviewer`, `generic_silent_failure_hunter`, `generic_type_design_analyzer`, plus the relevant domain specialist |
