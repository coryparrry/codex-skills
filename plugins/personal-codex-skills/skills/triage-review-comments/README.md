...
This skill loads the full PR review context, builds a complete inventory, deduplicates by underlying issue, verifies claims against current code before trusting them, classifies everything into four buckets, resolves inline threads that are already fixed, tracks real deferred work, and recommends prevention tests so the same issues don't come back.
...
- Classifies every actionable comment into one of four buckets:
  ...
  - **Fix if cheap** — verified valid, limited impact, low-risk to take now
...
- Files real deferred non-Binder items as Linear issues under the correct project
- Records real Binder deferred items in `docs/Deferred bugs.md` after approval
...
Recommended: install the `personal-codex-skills` plugin from this repository's marketplace source in the Codex app:
...
# Install the skill
cp -r triage-review-comments ~/.codex/skills/triage-review-comments
```

Then restart Codex. No dependencies.
