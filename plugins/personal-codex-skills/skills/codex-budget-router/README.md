...
> Deprecated: `codex-budget-router` needs a model-routing update before use. Its routing table and agent profiles still reference removed GPT-5.3 Codex models, including `gpt-5.3-codex` and `gpt-5.3-codex-spark`.
...
`codex-budget-router` changed that. It's a routing policy that kept the root model focused on architecture, judgment, and integration, while moving broad search, routine implementation, tests, and bounded review to cheaper workers. It saved me up to 20% of my limits by displacing expensive-model work, not by reducing total tokens.

The skill is currently preserved as a historical reference only. Do not use it as-is until the model table, worker profiles, and audit logic are updated for currently available Codex models.
...
Deprecated routing policy for moving suitable work to cheaper bounded workers while keeping final judgment in the root session.
...
- Historically used `spark_worker` (gpt-5.3-codex-spark) first for low-risk routine work — tests, scripts, fixtures, docs cleanup, mechanical refactors
- Historically used `codex_worker` (gpt-5.3-codex) for bounded non-trivial implementation after the root has decided the approach
...
This skill is deprecated and should not be installed for active routing until its model references are updated.
