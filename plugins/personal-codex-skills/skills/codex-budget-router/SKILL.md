...
description: Deprecated budget-aware Codex routing skill. Needs an update before use because the GPT-5.3 Codex routes it depends on have been removed.
---

# Codex Budget Router

> Deprecated: this skill needs a model-routing update before use. Its routing table and agent profiles still reference removed GPT-5.3 Codex models, including `gpt-5.3-codex` and `gpt-5.3-codex-spark`.

Do not use this skill as-is for active routing. Treat it as a historical reference until the model table, worker profiles, and audit logic are updated for currently available Codex models.
