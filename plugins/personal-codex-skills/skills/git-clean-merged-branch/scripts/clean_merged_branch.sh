...
verify_squash_merged_branch() {
  ...
  pr_info="$(
    gh pr list \
      ...
      --jq '.[] | [.number, .url, .mergedAt, (.mergeCommit.oid // "")] | @tsv' 2>/dev/null || true
  )"
  ...
  local pr_number pr_url merged_at merge_oid
  IFS=$'\t' read -r pr_number pr_url merged_at merge_oid <<< "$pr_info"
