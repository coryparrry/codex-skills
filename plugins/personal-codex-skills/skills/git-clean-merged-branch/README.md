...
clean_merged_branch.sh
  ...
  ├─ git branch -d <old-branch>   (or verified -D after squash/rebase merge)
...
# Install the skill
cp -r git-clean-merged-branch ~/.codex/skills/git-clean-merged-branch
```
...
To keep the old remote branch:

```bash
bash ~/.codex/skills/git-clean-merged-branch/scripts/clean_merged_branch.sh --keep-remote
```
