...
3. Choose the relevant route from `.repo/graph.json`.
4. Inspect the route's `inspect_first` paths before broad search.
5. Follow semantic nodes only when the route lists `start_nodes`.
...
- Treat `.repo/graph.json` as routes-first, evidence-backed navigation, not source truth.
