...
class CleanMergedBranchTests(unittest.TestCase):
    ...
    def test_does_not_delete_remote_branch_when_remote_tip_has_unverified_commits(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            origin = root / "origin.git"
            work = root / "work"
            ...
            self.run_git("config", "user.email", "test@example.com", cwd=other)
            self.run_git("config", "user.name", "Test", cwd=other)
            self.run_git("switch", "feature", cwd=other)
            (other / "unmerged.txt").write_text("remote only\n")
            self.run_git("add", "unmerged.txt", cwd=other)
            self.run_git("commit", "-m", "remote-only feature work", cwd=other)
            self.run_git("push", "origin", "feature", cwd=other)

            self.run_git("switch", "feature", cwd=work)
            gh = bin_dir / "gh"
            gh.write_text(
                "#!/usr/bin/env bash\n"
                f"printf '3\\thttps://example.test/pr/3\\t2026-06-05T00:00:00Z\\t{merge_oid}\\t1\\n'\n"
            )
            gh.chmod(0o755)
            ...
            self.assertIn("Deleted old local branch: feature", result.stdout)
            self.assertIn("Old remote branch was not deleted.", result.stdout)
            self.assertIn("points at unverified commits", result.stderr)
            self.assertNotIn("feature", self.run_git("branch", "--list", cwd=work).stdout)
            self.assertNotEqual("", self.run_git("ls-remote", "--heads", "origin", "feature", cwd=work).stdout)

    def test_force_local_delete_does_not_delete_matching_unverified_remote(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            origin = root / "origin.git"
            work = root / "work"
            ...
            self.run_git("switch", "-c", "feature", cwd=work)
            (work / "unmerged.txt").write_text("local only\n")
            self.run_git("add", "unmerged.txt", cwd=work)
            self.run_git("commit", "-m", "unmerged feature work", cwd=work)
            self.run_git("push", "-u", "origin", "feature", cwd=work)
