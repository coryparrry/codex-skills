...
assert_installed() {
  ...
  test -f "$codex_home/skills/luna-advisor/SKILL.md"
  test -f "$codex_home/skills/luna-advisor/agents/openai.yaml"
  ...
  test "$installed_skills" = "$(printf '%s\n' appstore-readiness-audit continue-deep-research deep-code-review engineering-advisor git-clean-merged-branch luna-advisor research-repo-technology swift-code-review triage-review-comments)"
}
...
before_luna_advisor_checksum="$(shasum "$PRESERVE_HOME/skills/luna-advisor/SKILL.md")"
...
after_luna_advisor_checksum="$(shasum "$PRESERVE_HOME/skills/luna-advisor/SKILL.md")"
...
test "$before_luna_advisor_checksum" = "$after_luna_advisor_checksum"
