...
Before inspecting the repository or running repository commands, read [model-and-reasoning-profiles.md](references/model-and-reasoning-profiles.md). Unless the current request already states both the coordinator model and reasoning level, ask one concise blocking question for the missing values. Ask which model and reasoning level should coordinate the review; state that specialist lanes default to risk-routed Luna, Terra, and Sol agents, and ask about that pool only when the user wants to restrict it. Do not infer either coordinator value from prior tasks, model availability, or a model name without an effort suffix. For Luna at `max` on a snapshot audit or repository-wide multi-agent review, immediately read [luna-max-whole-repository-audit.md](references/luna-max-whole-repository-audit.md) and treat it as the controlling orchestration contract.
